...
export function findScheduleConflict(candidate, records, ignoreId = "") {
  ...
  const candidateHandover = normalizeValue(getHandoverPerson(candidate));
  ...
  const conflict = records.find((record) => {
    ...
    const sameHandover = candidateHandover && normalizeValue(getHandoverPerson(record)) === candidateHandover;
    return sameMusaedah || sameHandover;
  });

  if (!conflict) return null;
  const sameHandover = candidateHandover && normalizeValue(getHandoverPerson(conflict)) === candidateHandover;
  return {
